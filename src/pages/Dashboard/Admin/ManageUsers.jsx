...
const ManageUsers = () => {
  ...
  const getRoleBadge = (role) => {
    const roleColors = {
      admin: 'bg-gradient-to-r from-rose-100 to-pink-100 text-rose-700 border border-rose-200',
      moderator: 'bg-gradient-to-r from-violet-100 to-purple-100 text-violet-700 border border-violet-200',
      student: 'bg-gradient-to-r from-blue-100 to-cyan-100 text-blue-700 border border-blue-200',
    };
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-slate-500 font-bold text-xs uppercase tracking-widest mt-1">Manage roles and user access</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-slate-500 text-xs font-black uppercase">Total Users</p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-slate-500 text-xs font-black uppercase">Admins</p>
                <p className="text-3xl font-black text-rose-600 mt-1">{users.filter(u => u.role?.toLowerCase() === 'admin').length}</p>
              </div>
              <div className="bg-rose-50 p-3 rounded-xl text-rose-600"><Crown size={24} /></div>
            </div>
          </div>
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-slate-500 text-xs font-black uppercase">Moderators</p>
                <p className="text-3xl font-black text-violet-600 mt-1">{users.filter(u => u.role?.toLowerCase() === 'moderator').length}</p>
              </div>
              <div className="bg-violet-50 p-3 rounded-xl text-violet-600"><Shield size={24} /></div>
            </div>
          </div>
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-slate-500 text-xs font-black uppercase">Students</p>
                <p className="text-3xl font-black text-blue-600 mt-1">{users.filter(u => u.role?.toLowerCase() === 'student').length}</p>
              </div>
