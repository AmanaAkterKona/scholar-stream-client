...
const AllReviews = () => {
  ...
  if (loading) {
    return (
      <div
        className={`flex flex-col justify-center items-center min-h-screen ${bgMain}`}
      >
        <div className="animate-spin rounded-full h-16 w-16 border-t-4 border-teal-600"></div>
        <p className="mt-4 text-teal-600 font-bold animate-pulse">
          Loading reviews...
        </p>
      </div>
    );
  ...
  return (
    <div
      className={`min-h-screen pb-20 transition-colors duration-300 ${bgMain}`}
    >
      {/* Hero Section */}
      <div
        className={`${
        ...
        } py-16 px-6 relative overflow-hidden mb-12`}
      >
        <div className="max-w-[1400px] mx-auto relative z-10 text-center md:text-left flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-teal-100 text-lg font-medium">
              Manage and moderate student feedback effectively.
            </p>
          </div>
          <div
            className={`${
              isDark ? "bg-slate-800" : "bg-white"
            } rounded-2xl shadow-2xl p-2 w-full md:w-[450px]`}
          >
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-12 pr-4 py-3 rounded-xl focus:outline-none font-bold ${
      ...
      <div className="max-w-[1400px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {[
            ...
            {
              label: "Results",
              val: filteredReviews.length,
              icon: <GraduationCap />,
              bg: isDark ? "bg-slate-800" : "bg-teal-900",
              text: "text-white",
            },
          ].map((stat, i) => (
            <div
              key={i}
              className={`${bgCard} p-6 rounded-3xl shadow-lg border flex items-center gap-4`}
            >
              <div
                className={`${stat.bg} ${stat.text} p-4 rounded-2xl shadow-lg`}
              >
                {stat.icon}
              </div>
              <div>
                <p
                  className={`${textSecondary} text-sm font-bold uppercase tracking-wider`}
                >
                  {stat.label}
                </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {currentItems.length === 0 ? (
            <div
              className={`col-span-full py-20 ${bgCard} rounded-3xl border-2 border-dashed text-center`}
            >
              <MessageSquare
                className={`w-16 h-16 ${
            ...
            currentItems.map((review) => (
              <div
                key={review._id}
                className={`${bgCard} group rounded-3xl p-8 shadow-lg border hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 flex flex-col`}
              >
                <div className="flex items-start gap-4 mb-6">
                  <img
                    src={
                      review.userImage || "https://i.ibb.co/5GzXkwq/user.png"
                    }
                    className={`w-16 h-16 rounded-2xl object-cover ring-4 ${
                      isDark ? "ring-slate-700" : "ring-teal-50"
                    } shadow-md group-hover:scale-105 transition-transform`}
                    alt=""
                  />
                  <div className="flex-1 overflow-hidden">
                    <h3
                      className={`font-black ${textPrimary} text-lg leading-tight truncate`}
                    >
                      {review.userName}
                    </h3>
                ...
                <div
                  className={`${
                    isDark ? "bg-slate-800/50" : "bg-teal-50/50"
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`text-xs font-black border-l pl-2 ${
                        isDark
                ...
                <div className="mb-8 min-h-[90px] relative">
                  <Quote
                    className={`absolute -top-2 -left-2 w-8 h-8 ${
                      isDark ? "text-slate-700" : "text-teal-100"
                    } opacity-50`}
                  />
                  <p
                    className={`${
                      isDark ? "text-slate-300" : "text-gray-700"
                    } text-[15px] font-medium leading-relaxed italic pl-4 border-l-2 border-teal-500 line-clamp-4`}
                  >
          ...
          <div className="flex justify-center items-center gap-2 mt-12">
            <button
              ...
              className={`p-2 rounded-xl transition-all ${
                isDark
                  ? "hover:bg-slate-800 text-slate-500 disabled:opacity-20"
