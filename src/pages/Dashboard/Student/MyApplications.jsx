  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const response = await axiosSecure.patch(
        `/applications/update/${selectedApp._id}`,
        {
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-semibold ${
                          isPending(app.applicationStatus)
                            ...
                            : isCancelled(app.applicationStatus)
                    ...
                    <td className="px-6 py-4 text-slate-600 max-w-[150px] truncate italic">
                      {app.feedback ? (
                        <span className="text-indigo-600 flex items-center gap-1">
                          <FaCommentDots className="shrink-0" /> {app.feedback}
                        </span>
                      ) : (
          ...
          <div className="bg-white rounded-xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-slate-600">
              <div>
                ...
                <p>{selectedApp.universityName}</p>
              </div>
              <div>
                <span className="font-bold text-slate-800">Subject:</span>{" "}
                <p>{selectedApp.subjectCategory}</p>
              </div>
              <div>
                <span className="font-bold text-slate-800">Location:</span>{" "}
                <p>
                  {selectedApp.universityCity}, {selectedApp.universityCountry}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="capitalize">{selectedApp.applicationStatus}</p>
              </div>
              <div>
                <span className="font-bold text-slate-800">
                  Payment Status:
                </span>{" "}
                <p className="capitalize">{selectedApp.paymentStatus}</p>
              </div>
            </div>
            ...
            <div className="mt-6 p-4 bg-indigo-50 rounded-lg border border-indigo-100">
              ...
              <p className="text-slate-700 italic leading-relaxed">
                {selectedApp.feedback ||
                  "No feedback provided yet by the moderator."}
              </p>
            </div>

            <button
              onClick={() => setShowDetailsModal(false)}
              className="mt-8 w-full bg-slate-800 text-white px-6 py-3 rounded-lg font-semibold hover:bg-slate-700"
            >
              Close Window
            </button>
          </div>
          ...
          <div className="bg-white rounded-xl p-8 max-w-lg w-full text-gray-700">
            ...
            <form onSubmit={handleEditSubmit}>
              ...
              <div className="flex gap-3 mt-6">
                <button
                  type="submit"
                  className="flex-1 bg-indigo-600 text-white px-6 py-3 rounded-lg font-semibold"
                >
                  Save Changes
                </button>
                <button
          ...
          <div className="bg-white rounded-xl p-8 max-w-lg w-full">
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 bg-purple-600 text-white px-6 py-3 rounded-lg font-semibold"
              >
                Submit
              </button>
              <button
